...
#define UART_16550_LCR_DLA_EN		(1 << 7)

#define UART_16550_IIR_PENDING		(0x1)
#define UART_16550_IIR_TX_EMPTY		(0x2)
#define UART_16550_IIR_RX_AVAIL		(0x4)

#define UART_16550_IER_RX_AVAIL_EN	(0x1)
#define UART_16550_IER_TX_EMPTY_EN	(0x2)
...
struct uart16550 {
	...
	DECLARE_KFIFO(write_fifo, u8, FIFO_SIZE);
...
static inline void _enable_irq(struct uart16550 *dev, u8 irq_mask)
{
	u8 ier = inb(UART_16550_IER(dev->io_base));

	ier |= irq_mask;
	outb(ier, UART_16550_IER(dev->io_base));
}

static inline void _disable_irq(struct uart16550 *dev, u8 irq_mask)
{
	u8 ier = inb(UART_16550_IER(dev->io_base));
...
static inline void _disable_interrupts(struct uart16550 *dev)
{
	_disable_irq(dev, UART_16550_IER_RX_AVAIL_EN);
	_disable_irq(dev, UART_16550_IER_TX_EMPTY_EN);
}

/* Writes 'struct uart16550_line_info' parameters to the HW */
static inline void _set_communication_params(struct uart16550 *dev)
{
...
static void _uart_16550_interrupt_handle_tx(struct uart16550 *dev)
{
	u8 thr;		/* THR - Transmit Holding Register */

	kfifo_get(&dev->write_fifo, &thr);
	outb(thr, UART_16550_THR(dev->io_base));

	/* Check if 'Transmitter Holding Register Empty Interrupt' should be turned off */
	if (kfifo_is_empty(&dev->write_fifo))
		_disable_irq(dev, UART_16550_IER_TX_EMPTY_EN);
...
static irqreturn_t uart_16550_interrupt_handle(int irq_no, void *dev_id)
{
	...
	if (!(iir & UART_16550_IIR_PENDING)) {
		if (iir & UART_16550_IIR_RX_AVAIL)
			...
			_uart_16550_interrupt_handle_tx(dev);
	}
...
uart16550_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	_enable_irq(dev, UART_16550_IER_RX_AVAIL_EN);
			 ...
			 size_t size, loff_t *offset)
{
	...
	if (copy_from_user(tmp_buffer, user_buffer, size) != 0) {
		written = -EFAULT;
		goto error;
	}
	...
	do {
		...
		wait_event(dev->write_wq, !kfifo_is_full_spinlocked(&dev->write_fifo, &dev->lock));

		spin_lock_irqsave(&dev->lock, flags);
		written = kfifo_in(&dev->write_fifo, tmp_buffer, size);

		/* Re-Enable TX IRQ */
		if (written > 0)
			_enable_irq(dev, UART_16550_IER_TX_EMPTY_EN);
		spin_unlock_irqrestore(&dev->lock, flags);
	} while (written == 0);
...
static int uart16550_init_minor(int minor)
{
	...
	INIT_KFIFO(devs[minor].write_fifo);
	...
	_enable_irq(&devs[minor], UART_16550_IER_RX_AVAIL_EN);
